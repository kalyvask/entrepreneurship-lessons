...
The PMF framework's discipline for validating a value hypothesis. The sequence runs concept test → implementation test → MVP test, each cheaper to run and more rigorous than the next. **Skip a phase only when you're confident the data from a prior phase makes the next one unambiguous.**
...
Synthesized from the PMF framework (validation as concept→implementation→MVP sequence); Jake Knapp's *Sprint*; Steve Blank's customer development.
